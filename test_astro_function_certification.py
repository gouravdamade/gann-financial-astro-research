...
def test_kaala_formula_profiles_are_diagnostic_and_fail_closed(
    ...
    for name, measure, passed in (
        ...
        ("nathonnatha_astronomical_midnight", "nathonnatha", 11),
    ...
    assert gate["lockedInputs"]["visibleComparison"]["actualSha256"] == (
        locked_inputs["visibleComparison"]["sha256"]
    )

    input_paths["visibleComparison"].write_text(
        "tampered\n",
        encoding="utf-8",
    )
    tampered = kaala_formula_profile_gate_summary(path)
    assert tampered["status"] == "blocked_incomplete_kaala_formula_profiles"
    assert (
        "visibleComparison locked input SHA-256 mismatch"
        in tampered["input"]["issues"]
    )
