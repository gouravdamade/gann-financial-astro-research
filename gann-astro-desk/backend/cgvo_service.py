...
def _g3_r1_source_composition_adjudication(project_root: Path) -> dict[str, Any]:
    ...
    required_false = (
        "downstreamIntersectionAuthorized", "eclipseVisibilityMatching", "spatialIntersectionAuthorized",
        "regionExtrapolationAuthorized", "chapterVEffectActivationAuthorized",
        "chapterXivChapterVCompositionAuthorized", "priceDataRead", "priceOutcomeRead", "marketUseAllowed",
        "marketDirectionInferred", "scoreAggregationUsed", "fieldsPath", "sbcPath", "autoSuggestPath",
        "mlPath", "mt5Path", "executionAllowed",
    )
    if any(guardrails.get(key) for key in required_false):
        raise RuntimeError("CGVO G3-R1 source-composition policy has an active prohibited path")
    if policy.get("chapterXivGeographyRole") != "CONTEXTUAL_PROVENANCE_ONLY":
    ...
    return {
        ...
        "chapterXIVGeographyRole": policy["chapterXivGeographyRole"],
        "siteVisibilityInferenceStatus": "SITE_ONLY",
        "regionVisibility": None,
        "regionVisibilityStatus": "REGION_EXTRAPOLATION_NOT_AUTHORIZED",
        "sourceEffectActivation": None,
        "sourceEffectActivationStatus": "CHAPTER_V_EFFECT_NOT_ACTIVATED",
...
def _g3_d1_reject_requested_scope(payload: Mapping[str, Any]) -> None:
    ...
    if requested_scope == "SOURCE_EFFECT" or any(
        bool(payload.get(key)) for key in ("sourceEffectActivation", "includeChapterVEffect")
    ):
        raise CgvoSiteVisibilityAuditError(
            "SOURCE_EFFECT_NOT_AUTHORIZED",
            "CGVO G3-R1 does not activate Chapter V effects from a local site-visibility audit",
        )
    if requested_scope in forbidden_scope_values or any(
        bool(payload.get(key))
        for key in ("regionVisibility", "includeRegion", "includeGandharaRegion", "spatialIntersection")
    ):
        raise CgvoSiteVisibilityAuditError(
...
def build_cgvo_site_visibility_audit(project_root: Path, payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    composition_adjudication = _g3_r1_source_composition_adjudication(project_root)
    ...
    return {
        ...
        "sourceCompositionAdjudication": composition_adjudication,
        "guardrails": {
            **policy["guardrails"],
            "regionExtrapolationAuthorized": False,
