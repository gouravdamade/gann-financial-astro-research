"""Outcome-blind S2R1 successor freeze after Saravali relationship lineage closure.
...
S2R1_HYPOTHESIS_REGISTRY_CONTRACT = "MACHINE_ASSISTED_EXPERIMENTAL_MARKET_HYPOTHESIS_REGISTRY_S2R1_V1"
S2R1_PREDICTION_FREEZE_CONTRACT = "MO_R4A_S2R1_BLINDED_MARKET_BRIDGE_PREDICTION_FREEZE_V1"
S2R1_INVARIANCE_AUDIT_CONTRACT = "MO_R4A_S2R1_BLINDED_MARKET_BRIDGE_INVARIANCE_AUDIT_V1"
S2R1_ACCEPTANCE_CONTRACT = "MO_R4A_S2R1_BLINDED_MARKET_BRIDGE_HYPOTHESIS_FREEZE_V1"
S2R1_LINEAGE_RECONCILIATION_CONTRACT = "MO_R4A_S2R1_SARAVALI_RELATIONSHIP_LINEAGE_RECONCILIATION_V1"
S2R1_EXPECTED_STARTING_MASTER = "8367908818c488fe92e4de8e0f86fb0b816c0462"
...
HISTORICAL_S2_REGISTRY_HASH = "E2FD1EA6B570F35E07F06259A64A19E181B06610F0659B1B333800F7688EBA66"
HISTORICAL_S2_FREEZE_HASH = "5A7D318D22D27637F2A253D362258648F40CF12DB6016B6743D166A1C010C27A"
S2R1_EXPECTED_LEDGER_HASH = "E9DB0C92B449045FB2ED35D482E3F7063C547CD2562305EEF778064C2F5558E6"
S2R1_EXPECTED_COVERAGE_HASH = "B22F42E78D20858045AE98F0010355E1E8895C60F1DD49570230ECC5B2C7F62A"
S2R1_HYPOTHESIS_FAMILY_ID = "MO_R4A_S2R1_COMPOUND_RELATIONSHIP_DIRECT_VALENCE_V1"
S2R1_HYPOTHESIS_IDS = {
    "USD": "H-MO-S2R1-COMPOUND-REL-DIRECT-USD-V1",
    "JPY": "H-MO-S2R1-COMPOUND-REL-DIRECT-JPY-V1",
}
S2R1_GUARDRAIL_KEYS = s2.S2_GUARDRAIL_KEYS
S2R1_SIDES = s2.S2_SIDES
...
DEFAULT_S2R1_REGISTRY_PATH = MACHINE_INTERPRETATION_ROOT / "experimental_market_hypothesis_registry_s2r1_v1.json"
DEFAULT_S2R1_LEDGER_PATH = SOURCE_OPERATOR_ROOT / "classical_source_operator_ledger_s2r1_saravali_lineage_v1.json"
DEFAULT_S2R1_COVERAGE_PATH = AUDIT_ROOT / "mo_r4a_s2r1_real_24_source_operator_coverage.json"
DEFAULT_S2R1_FREEZE_PATH = AUDIT_ROOT / "mo_r4a_s2r1_blinded_market_bridge_prediction_freeze.json"
DEFAULT_S2R1_INVARIANCE_AUDIT_PATH = AUDIT_ROOT / "mo_r4a_s2r1_blinded_market_bridge_invariance_audit.json"
DEFAULT_S2R1_RECONCILIATION_PATH = AUDIT_ROOT / "mo_r4a_s2r1_saravali_relationship_lineage_reconciliation.json"
DEFAULT_S2R1_ACCEPTANCE_PATH = ACCEPTANCE_ROOT / "mo_r4a_s2r1_blinded_market_bridge_hypothesis_freeze.json"
DEFAULT_S2R1_REVIEW_TABLE_PATH = DOCS_ROOT / "MULTI_OSCILLATOR_MO_R4A_S2R1_REAL_24_EVENT_PREDICTION_FREEZE.md"


class BlindedMarketBridgeS2R1Error(ValueError):
...
def _read_json(path: Path, label: str) -> dict[str, Any]:
    try:
        ...
        raise BlindedMarketBridgeS2R1Error(f"Missing {label}: {path}") from exc
    except json.JSONDecodeError as exc:
        raise BlindedMarketBridgeS2R1Error(f"Invalid JSON in {label}: {path}") from exc
    if not isinstance(raw, dict):
        raise BlindedMarketBridgeS2R1Error(f"{label} must be a JSON object")
    return raw
...
def _exact_keys(mapping: Any, expected: set[str], label: str) -> dict[str, Any]:
    if not isinstance(mapping, dict) or set(mapping) != expected:
        raise BlindedMarketBridgeS2R1Error(f"{label} keys do not match its bounded contract")
    return mapping
...
def _assert_hash(value: Any, label: str) -> str:
    ...
    if len(value) != 64 or any(character not in "0123456789ABCDEF" for character in value):
        raise BlindedMarketBridgeS2R1Error(f"{label} must be an uppercase SHA-256")
    return value
...
def _load_registry(path: Path) -> s2.S2HypothesisRegistry:
    expected = {
        ...
        "historicalS2RegistryHash", "historicalS2PredictionFreezeHash", "sourceLineageCorrectionReason",
    ...
    raw = _exact_keys(_read_json(path, "S2R1 market hypothesis registry"), expected, "S2R1 market hypothesis registry")
    if (
        raw["contract"], raw["schemaVersion"], raw["registryStatus"], raw["startingMaster"], raw["hypothesisFamilyId"]
    ) != (
        S2R1_HYPOTHESIS_REGISTRY_CONTRACT, 1, "AUTHORIZED_FOR_BLINDED_SUCCESSOR_REFREEZE", S2R1_EXPECTED_STARTING_MASTER,
        S2R1_HYPOTHESIS_FAMILY_ID,
    ):
        raise BlindedMarketBridgeS2R1Error("S2R1 registry has an unauthorized identity")
    if (raw["historicalS2RegistryHash"], raw["historicalS2PredictionFreezeHash"]) != (
        HISTORICAL_S2_REGISTRY_HASH, HISTORICAL_S2_FREEZE_HASH
    ):
        raise BlindedMarketBridgeS2R1Error("S2R1 registry does not preserve the historical S2 bindings")
    if (raw["upstreamSourceLedgerContract"], raw["upstreamSourceLedgerHash"], raw["upstreamCoverageContract"], raw["upstreamCoverageHash"]) != (
        operators.CLASSICAL_SOURCE_OPERATOR_S2R1_LEDGER_CONTRACT, S2R1_EXPECTED_LEDGER_HASH,
        operators.CLASSICAL_SOURCE_OPERATOR_S2R1_COVERAGE_CONTRACT, S2R1_EXPECTED_COVERAGE_HASH,
    ):
        raise BlindedMarketBridgeS2R1Error("S2R1 registry is not bound to the exact Saravali successor source artifacts")
    s2._require_false_flags(raw["guardrails"], S2R1_GUARDRAIL_KEYS, "S2R1 registry guardrails")
    entries_raw = raw["entries"]
    if not isinstance(entries_raw, list) or len(entries_raw) != 2:
        raise BlindedMarketBridgeS2R1Error("S2R1 registry requires exactly two side-local hypotheses")
    ...
    expected_entry_keys = {
        ...
        "supersedesHypothesisId", "supersedesHypothesisHash", "sourceLineageCorrectionReason",
        "newUpstreamSourceLedgerHash", "newUpstreamCoverageHash", "hypothesisHash",
    }
    historical_ids = s2.S2_HYPOTHESIS_IDS
    for entry in entries_raw:
        entry = _exact_keys(entry, expected_entry_keys, "S2R1 hypothesis")
        ...
        if (
            ...
            S2R1_HYPOTHESIS_FAMILY_ID, "1", "SOURCE_OPERATOR_STATE_PREDICATE", [s2.S2_PRIMARY_OPERATOR_ID],
            ...
            False, historical_ids.get(side), S2R1_EXPECTED_LEDGER_HASH, S2R1_EXPECTED_COVERAGE_HASH,
        ):
            raise BlindedMarketBridgeS2R1Error("S2R1 hypothesis exceeds its bounded direct compound-state bridge")
        _assert_hash(entry["supersedesHypothesisHash"], "S2R1 supersedesHypothesisHash")
        if entry["hypothesisHash"] != s2._canonical_hash(_entry_content(entry)):
            raise BlindedMarketBridgeS2R1Error("S2R1 hypothesis hash does not bind the successor decision content")
    ...
    if set(by_side) != set(S2R1_SIDES) or {entry.hypothesis_id for entry in entries} != set(S2R1_HYPOTHESIS_IDS.values()):
        raise BlindedMarketBridgeS2R1Error("S2R1 registry must contain one approved USD and one approved JPY successor")
    if raw["registryHash"] != s2._canonical_hash({key: copy.deepcopy(value) for key, value in raw.items() if key != "registryHash"}):
        raise BlindedMarketBridgeS2R1Error("S2R1 registry hash does not bind its frozen contents")
...
def _load_verified_upstream_inputs(root: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ledger = _read_json(root / DEFAULT_S2R1_LEDGER_PATH.relative_to(PROJECT_ROOT), "S2R1 successor ledger")
    coverage = _read_json(root / DEFAULT_S2R1_COVERAGE_PATH.relative_to(PROJECT_ROOT), "S2R1 successor coverage")
    if (ledger.get("contract"), s2._canonical_hash(ledger)) != (operators.CLASSICAL_SOURCE_OPERATOR_S2R1_LEDGER_CONTRACT, S2R1_EXPECTED_LEDGER_HASH):
        raise BlindedMarketBridgeS2R1Error("S2R1 ledger has an unexpected source lineage binding")
    ...
    if (
        ...
        operators.CLASSICAL_SOURCE_OPERATOR_S2R1_COVERAGE_CONTRACT, S2R1_EXPECTED_COVERAGE_HASH, S2R1_EXPECTED_COVERAGE_HASH,
        S2R1_EXPECTED_LEDGER_HASH,
    ):
        raise BlindedMarketBridgeS2R1Error("S2R1 coverage has an unexpected source lineage binding")
    if ledger != operators.build_s2r1_saravali_lineage_ledger(root) or coverage != operators.build_s2r1_real_24_source_operator_coverage(root):
        raise BlindedMarketBridgeS2R1Error("S2R1 checked-in source artifacts do not match deterministic source rebinding")
    for key in ("reviewStoreRead", "founderDecisionRead", "priceOrOutcomeRead", "eventUniverseRegenerated"):
        if coverage.get(key) is not False:
            raise BlindedMarketBridgeS2R1Error(f"S2R1 coverage guardrail is no longer false: {key}")
    return ledger, coverage


def build_s2r1_blinded_prediction_freeze(resource_root: Path = PROJECT_ROOT) -> dict[str, Any]:
    """Freeze the successor's two side-local predictions from corrected source lineage only."""

    root = Path(resource_root).resolve()
    registry = _load_registry(root / DEFAULT_S2R1_REGISTRY_PATH.relative_to(PROJECT_ROOT))
    ...
    if (
        ...
        raise BlindedMarketBridgeS2R1Error("S2R1 registry is not bound to its checked-in successor source coverage")
    ...
    for coverage_side in coverage["sides"]:
        ...
        if not isinstance(identities, list) or len(events) != len(identities) != 12:
            raise BlindedMarketBridgeS2R1Error(f"S2R1 coverage/identity order mismatch for {side}")
    ...
    if len(predictions) != 24 or len({item["eventId"] for item in predictions}) != 24:
        raise BlindedMarketBridgeS2R1Error("S2R1 requires exactly 24 unique frozen events")
    per_side = [s2._side_counts(predictions, side) for side in S2R1_SIDES]
    summary = {
        "eventCount": 24,
    ...
    if tuple(summary[key] for key in ("eventCount", "usdEventCount", "jpyEventCount", "singlePassVerifiedCount", "bridgeEligibleCount", "directionalCount", "neutralCount", "abstentionCount")) != (24, 12, 12, 24, 17, 14, 3, 7):
        raise BlindedMarketBridgeS2R1Error("S2R1 successor has an unexpected frozen pilot shape")
    body = {
        "contract": S2R1_PREDICTION_FREEZE_CONTRACT,
        "schemaVersion": 1,
        "milestone": "MO-R4A-S2R1",
        "startingMaster": S2R1_EXPECTED_STARTING_MASTER,
        "inputPolicy": "FROZEN_S2R1_SARAVALI_LINEAGE_COVERAGE_PLUS_IMMUTABLE_BLANK_PACKET_IDENTITY_ONLY",
        ...
        "historicalS2RegistryHash": HISTORICAL_S2_REGISTRY_HASH,
        "historicalS2PredictionFreezeHash": HISTORICAL_S2_FREEZE_HASH,
        ...
        "hypothesisIds": [registry.entries_by_side[side].hypothesis_id for side in S2R1_SIDES],
        "frozenEventOrder": [item["eventId"] for item in predictions],
        "predictions": predictions,
        "summary": summary,
        "perSide": per_side,
        "guardrails": {
            **{key: False for key in S2R1_GUARDRAIL_KEYS},
...
def _historical_s2_freeze(root: Path) -> dict[str, Any]:
    historical = _read_json(root / "status" / "audits" / "mo_r4a_s2_blinded_market_bridge_prediction_freeze.json", "historical S2 freeze")
    body = {key: copy.deepcopy(value) for key, value in historical.items() if key != "predictionFreezeHash"}
    if historical.get("predictionFreezeHash") != HISTORICAL_S2_FREEZE_HASH or s2._canonical_hash(body) != HISTORICAL_S2_FREEZE_HASH:
        raise BlindedMarketBridgeS2R1Error("Historical S2 prediction freeze must remain immutable")
    return historical


def build_s2r1_invariance_audit(
    resource_root: Path = PROJECT_ROOT,
    *,
    prediction_freeze: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
    """Bind every S2R1 result to the same frozen identity and astronomy snapshot."""

    root = Path(resource_root).resolve()
    _, coverage = _load_verified_upstream_inputs(root)
    freeze = copy.deepcopy(prediction_freeze) if prediction_freeze is not None else build_s2r1_blinded_prediction_freeze(root)
    if freeze.get("predictionFreezeHash") != s2._canonical_hash({key: value for key, value in freeze.items() if key != "predictionFreezeHash"}):
        raise BlindedMarketBridgeS2R1Error("S2R1 freeze hash does not bind frozen content")
    ...
    if len(coverage_events) != len(freeze["predictions"]) != 24:
        raise BlindedMarketBridgeS2R1Error("S2R1 invariance audit requires 24 source events and predictions")
    rows = []
    ...
    if not all(row["identityUnchanged"] and row["astronomySnapshotUnchanged"] and row["overallSourceCompositionUnchanged"] for row in rows):
        raise BlindedMarketBridgeS2R1Error("S2R1 successor changed an event identity, astronomy snapshot, or overall unknown composition")
    body = {
        "contract": S2R1_INVARIANCE_AUDIT_CONTRACT,
        "schemaVersion": 1,
        "milestone": "MO-R4A-S2R1",
...
def build_s2r1_lineage_reconciliation(
    resource_root: Path = PROJECT_ROOT,
    *,
    ...
    root = Path(resource_root).resolve()
    historical = _historical_s2_freeze(root)
    _, corrected_coverage = _load_verified_upstream_inputs(root)
    successor = copy.deepcopy(successor_freeze) if successor_freeze is not None else build_s2r1_blinded_prediction_freeze(root)
    comparison = operators.build_s2r1_relationship_matrix_comparison(root)
    historical_coverage = _read_json(root / "status" / "audits" / "mo_r4a_s1r1_r1_real_24_source_operator_coverage.json", "historical S1R1-R1 coverage")
    ...
    for old_prediction, new_prediction, old_event, new_event in zip(historical["predictions"], successor["predictions"], old_events, new_events, strict=True):
        old_natural = next(item for item in old_event["operatorOutputs"] if item["operatorId"] == "TRAILOKYA_1972_NATURAL_RELATIONSHIP_V1")
        ...
        if compound_changed or prediction_changed:
            reason = "SARAVALI_VS_TRAILOKYA_NATURAL_RELATIONSHIP_CONFLICT_REBOUND"
        else:
            reason = "SOURCE_LINEAGE_REBOUND_ONLY; verified MERCURY_TO_MARS conflict is absent from this frozen event."
        rows.append(
            {
                ...
                "historicalS2NaturalSource": {"operatorId": old_natural["operatorId"], "outputState": old_natural["outputState"]},
                "correctedNaturalSource": {"operatorId": new_natural["operatorId"], "outputState": new_natural["outputState"]},
                ...
                "historicalS2CompoundState": old_compound,
                "correctedCompoundState": new_compound,
                ...
                "historicalS2PressureState": old_prediction["currencyPressureState"],
                "correctedPressureState": new_prediction["currencyPressureState"],
                "historicalBridgeApplied": old_prediction["bridgeApplied"],
                "correctedBridgeApplied": new_prediction["bridgeApplied"],
    ...
    if len(rows) != 24 or any(row["eventId"] != successor["frozenEventOrder"][index] for index, row in enumerate(rows)):
        raise BlindedMarketBridgeS2R1Error("S2R1 lineage diff must preserve every frozen event exactly once in order")
    summary = {
        ...
        "changedCompoundEventCount": sum(row["historicalS2CompoundState"] != row["correctedCompoundState"] for row in rows),
    ...
    body = {
        "contract": S2R1_LINEAGE_RECONCILIATION_CONTRACT,
        "schemaVersion": 1,
        "milestone": "MO-R4A-S2R1",
        "startingMaster": S2R1_EXPECTED_STARTING_MASTER,
        ...
        "historicalS2RegistryHash": HISTORICAL_S2_REGISTRY_HASH,
        "successorS2RegistryHash": successor["hypothesisRegistryHash"],
        "historicalS2PredictionFreezeHash": HISTORICAL_S2_FREEZE_HASH,
        "successorS2PredictionFreezeHash": successor["predictionFreezeHash"],
        "eventDiff": rows,
        "summary": summary,
        "priceDataRead": False,
        "outcomeDataRead": False,
...
def build_s2r1_acceptance_manifest(
    resource_root: Path = PROJECT_ROOT,
    *,
    prediction_freeze: Mapping[str, Any] | None = None,
    ...
    registry = _load_registry(root / DEFAULT_S2R1_REGISTRY_PATH.relative_to(PROJECT_ROOT))
    freeze = copy.deepcopy(prediction_freeze) if prediction_freeze is not None else build_s2r1_blinded_prediction_freeze(root)
    audit = copy.deepcopy(invariance_audit) if invariance_audit is not None else build_s2r1_invariance_audit(root, prediction_freeze=freeze)
    lineage = copy.deepcopy(reconciliation) if reconciliation is not None else build_s2r1_lineage_reconciliation(root, successor_freeze=freeze)
    if (freeze["hypothesisRegistryHash"], audit["predictionFreezeHash"], lineage["successorS2PredictionFreezeHash"]) != (
        registry.registry_hash, freeze["predictionFreezeHash"], freeze["predictionFreezeHash"]
    ):
        raise BlindedMarketBridgeS2R1Error("S2R1 acceptance inputs are not bound to one successor freeze")
    body = {
        "contract": S2R1_ACCEPTANCE_CONTRACT,
        "schemaVersion": 1,
        "milestone": "MO-R4A-S2R1",
        "status": "S2_SOURCE_LINEAGE_RECONCILED_AND_BLINDED_REFREEZE_COMPLETE_CENTRAL_REVIEW_REQUIRED",
...
def render_s2r1_prediction_freeze_markdown(freeze: Mapping[str, Any], reconciliation: Mapping[str, Any]) -> str:
    """Render a source-only review table and the all-24 lineage diff without outcomes."""

    if freeze.get("contract") != S2R1_PREDICTION_FREEZE_CONTRACT:
        raise BlindedMarketBridgeS2R1Error("Cannot render an unsupported S2R1 freeze")
    ...
    lines = [
        ...
        f"Historical S2 prediction freeze hash: `{HISTORICAL_S2_FREEZE_HASH}`",
    ...
    for row in reconciliation["eventDiff"]:
        lines.append(
            f"| {row['sideIdentity']} | {row['eventId']} | {row['historicalS2CompoundState']} | {row['correctedCompoundState']} | {row['historicalS2PressureState']} | {row['correctedPressureState']} | {str(row['predictionChanged']).lower()} |"
        )
    lines.extend(
        [
            "",
...
def write_s2r1_artifacts(resource_root: Path = PROJECT_ROOT) -> dict[str, Path]:
    """Materialize deterministic successor artifacts after source-only rebinding."""

    root = Path(resource_root).resolve()
    operators.write_s2r1_source_lineage_artifacts(root)
    freeze = build_s2r1_blinded_prediction_freeze(root)
    audit = build_s2r1_invariance_audit(root, prediction_freeze=freeze)
    reconciliation = build_s2r1_lineage_reconciliation(root, successor_freeze=freeze)
    acceptance = build_s2r1_acceptance_manifest(root, prediction_freeze=freeze, invariance_audit=audit, reconciliation=reconciliation)
    artifacts = {
        "predictionFreeze": root / DEFAULT_S2R1_FREEZE_PATH.relative_to(PROJECT_ROOT),
        "invarianceAudit": root / DEFAULT_S2R1_INVARIANCE_AUDIT_PATH.relative_to(PROJECT_ROOT),
        "reconciliation": root / DEFAULT_S2R1_RECONCILIATION_PATH.relative_to(PROJECT_ROOT),
        "acceptance": root / DEFAULT_S2R1_ACCEPTANCE_PATH.relative_to(PROJECT_ROOT),
        "reviewTable": root / DEFAULT_S2R1_REVIEW_TABLE_PATH.relative_to(PROJECT_ROOT),
    }
    payloads = {
    ...
    artifacts["reviewTable"].write_text(render_s2r1_prediction_freeze_markdown(freeze, reconciliation), encoding="utf-8")
    return artifacts
