...
class ChakraLabServiceTests(unittest.TestCase):
    ...
    def test_trailokya_source_only_geometry_rejects_the_generic_grid(self) -> None:
        with self.assertRaisesRegex(
            ValueError,
            "TRAILOKYA_SOURCE_NATIVE_GRID_ADAPTER_REQUIRED",
        ):
            build_chakra_lab_trailokya_source_only_geometry(
                {
                    "at": "2026-07-17T12:00:00+05:30",
    ...
    def test_atomic_range_remains_an_independent_read_only_timeline(self) -> None:
        atomic_range = build_chakra_lab_atomic_range(
            self._two_interval_audit_request()
        )

        self.assertEqual(atomic_range["contract"], "SBC_ATOMIC_VISIBLE_RANGE_V1")
        self.assertEqual(atomic_range["schema_version"], 1)
        self.assertEqual(atomic_range["field_role"], "INDEPENDENT_SYNCHRONIZED_COMPARISON")
        self.assertEqual(atomic_range["aspect_relationship"], "NOT_AUTOMATIC_CONFIRMATION")
        self.assertEqual(atomic_range["magnitude_state"], "NOT_CONFIGURED")
        self.assertEqual(len(atomic_range["intervals"]), 2)
        self.assertEqual(
            atomic_range["intervals"][0]["end_utc"],
            atomic_range["intervals"][1]["start_utc"],
        )
        self.assertFalse(atomic_range["guardrails"]["acts_as_aspect_confirmation"])
        self.assertFalse(atomic_range["guardrails"]["execution_allowed"])
        self.assertFalse(atomic_range["guardrails"]["automatic_order_placement"])

    def test_atomic_range_retains_missing_guidance_as_unknown(self) -> None:
        atomic_range = build_chakra_lab_atomic_range(
            {
                "instrumentIdentity": "FX:USDJPY",
        ...
        self.assertEqual(
            atomic_range["intervals"][0]["guidance_availability"],
            "UNKNOWN",
        )
        self.assertIn(
            "VEDHA_GUIDANCE_NOT_AVAILABLE",
            atomic_range["intervals"][0]["missing_evidence_ids"],
        )
    ...
    def test_timing_external_review_reports_missing_attestation(self) -> None:
        review = build_chakra_lab_timing_external_review(
            {
                "reviewBundle": None,
                "attestation": None,
            }
        ...
        self.assertEqual(review["review_status"], "NO_ATTESTATION")
        self.assertFalse(review["bundle_integrity_verified"])
        self.assertFalse(review["attestation_complete"])
        self.assertFalse(review["ready_for_human_certification_decision"])
        self.assertIsNone(review["certification_proposal"])
        self.assertFalse(review["source_certified"])
        self.assertFalse(review["registry_write_allowed"])
        self.assertFalse(review["guardrails"]["execution_allowed"])

    def test_timing_external_review_rejects_outer_unknown_fields(self) -> None:
        with self.assertRaisesRegex(ValueError, "Unknown"):
            build_chakra_lab_timing_external_review(
                {
                    "reviewBundle": None,
    ...
    def test_timing_signed_review_reports_missing_s3_evidence(self) -> None:
        review = build_chakra_lab_timing_signed_review(
            {
                "reviewBundle": None,
                "attestation": None,
                "signedReview": None,
        ...
        self.assertEqual(
            review["contract"],
            "SBC_TIMING_PROFILE_SIGNED_REVIEW_REPORT_V1",
        )
        self.assertEqual(review["review_status"], "S3_NOT_READY")
        self.assertFalse(review["s3_ready"])
        self.assertTrue(review["reviewer_registry_valid"])
        self.assertFalse(review["reviewer_key_trusted"])
        self.assertFalse(review["review_signature_valid"])
        self.assertFalse(review["ready_for_manual_source_certification"])
        self.assertFalse(review["source_certified"])
        self.assertFalse(review["registry_write_allowed"])
        self.assertFalse(review["guardrails"]["execution_allowed"])
        self.assertFalse(review["guardrails"]["client_public_key_accepted"])

    def test_timing_signed_review_rejects_registry_or_public_key_inputs(
        self,
    ) -> None:
        with self.assertRaisesRegex(ValueError, "Unknown"):
            build_chakra_lab_timing_signed_review(
