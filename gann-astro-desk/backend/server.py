...
@app.post("/api/chakra-lab/atomic-range")
def create_chakra_lab_atomic_range() -> Any:
    try:
        payload = request.get_json(force=True, silent=False)
        return jsonify(
            {
                "ok": True,
                "range": build_chakra_lab_atomic_range(payload),
            }
        )
    except (TypeError, ValueError) as exc:
        return jsonify({"ok": False, "error": str(exc)}), 400
...
@app.post("/api/independent-fields/synchronized-range")
def create_synchronized_independent_range() -> Any:
    try:
        payload = request.get_json(force=True, silent=False)
        return jsonify(
            {
                "ok": True,
                "range": build_synchronized_independent_range(payload),
            }
        )
    except (TypeError, ValueError) as exc:
        return jsonify({"ok": False, "error": str(exc)}), 400
...
@app.post("/api/independent-fields/pilot-status")
def create_fx_side_pilot_status() -> Any:
    try:
        payload = request.get_json(force=True, silent=False)
