...
class CgvoS1bSourceAuditTests(unittest.TestCase):
    ...
    def test_firmament_remains_raw_geometry_without_downstream_classifier(self) -> None:
        ...
        self.assertEqual(status["milestones"]["sourceComposition"], "CGVO-G3-S1-R1")
        self.assertEqual(status["sourceCompositionAdjudication"]["siteToRegionInference"], "NOT_AUTHORIZED")
        self.assertEqual(status["sourceCompositionAdjudication"]["rootWitnessStatus"], "ACQUIRED_CHECKSUM_VERIFIED")
        self.assertEqual(status["sourceCompositionAdjudication"]["chapterVtoXivReferenceStatus"], "SOURCE_CLOSED_ROOT_KURMA_REFERENCE")
