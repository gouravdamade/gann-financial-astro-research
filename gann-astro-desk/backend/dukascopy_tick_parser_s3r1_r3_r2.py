"""Offline-only exact-single-stream parser for the R3-R2 USDJPY BI5 format.
...
PARSER_CONTRACT = "MO_R4A_S3R1_R3_R2_DUKASCOPY_TICK_PARSER_CONTRACT_V1"
...
def _decompress_single_frozen_format(raw_bytes: bytes) -> bytes:
    ...
    try:
        decoded = decoder.decompress(raw_bytes)
    except lzma.LZMAError as exc:
        raise DukascopyTickParserError("COMPRESSION_DECODE_FAILED", "frozen LZMA decompression failed") from exc
    if not decoder.eof:
        raise DukascopyTickParserError(
            "COMPRESSION_STREAM_INCOMPLETE",
            "the frozen LZMA stream did not reach its end marker",
        )
    if decoder.unused_data:
        raise DukascopyTickParserError(
            "COMPRESSION_TRAILING_DATA",
            "the frozen LZMA stream contains trailing or concatenated compressed data",
        )
    return decoded
