...
import { HistoricalFamilyEvidence } from '../components/HistoricalFamilyEvidence'
      ...
      <section className="analyze-layout">
        ...
        <section className="analyze-chart-column">
          ...
          <div className="analyze-chart-row">
            ...
            <MarketChart
              ...
              onShowAspectDetails={(aspect) => {
                if (aspect.familyKey === family.familyKey) {
                  setSelectedEventId(aspect.eventId)
                  setTab('evidence')
                }
              }}
          ...
          <footer className="family-summary-strip">
            ...
            <span><strong>{family.summary.bullishRatePct == null ? '-' : `${family.summary.bullishRatePct.toFixed(1)}%`}</strong> bullish rate</span>
            <span><strong>{family.summary.medianReturnPct == null ? '-' : `${family.summary.medianReturnPct > 0 ? '+' : ''}${family.summary.medianReturnPct}%`}</strong> median 72h</span>
          </footer>
        </section>
        <aside className="analysis-inspector">
          ...
          {tab === 'evidence' && (
            <div className="analysis-tab-body">
              ...
              <EvidenceCertificationGrid items={detail.evidenceCertifications} />
              ...
              {detail.currencyPairEvidence && (
                <section className="analysis-pair-evidence">
                  <header>
                    <strong>{detail.currencyPairEvidence.base.label}/{detail.currencyPairEvidence.quote.label} relative evidence</strong>
                    <span>{detail.currencyPairEvidence.status.replaceAll('_', ' ')}</span>
                  </header>
                  <CurrencyDivergence evidence={detail.currencyPairEvidence} />
                    ...
                    {[detail.currencyPairEvidence.base, detail.currencyPairEvidence.quote].map((side) => (
                      <div key={side.label}>
                        ...
                        <dl>
                          ...
                          <dt>Doctrine</dt><dd>{side.doctrineNetScore?.toFixed(3) ?? '-'}</dd>
                          <dt>Hits</dt><dd>{side.scoredHitCount ?? 0}</dd>
                          <dt>Dominant</dt><dd>{side.doctrineDominantHit || side.dominantHit || '-'}</dd>
                          <dt>Dignity</dt><dd>{side.doctrineDominantDignity || '-'}</dd>
                        </dl>
                      </div>
                    ))}
                  </div>
                  ...
                  <p>Provisional research evidence. Local LLM commentary is shown separately and remains draft until verified.</p>
                </section>
              )}
