...
export type ChartConditionedPolarityRangeInterval = {
  intervalId: string
  startUtc: string
  endUtc: string
  polarityState: 'SUPPORTIVE' | 'ADVERSE' | 'MIXED' | 'NEUTRAL' | 'UNKNOWN'
...
export type ChartConditionedPolarityRange = {
  contract: 'CHART_CONDITIONED_CATEGORICAL_RANGE_V1'
  schemaVersion: 1
  instrumentId: string
  sideIdentity: 'USD' | 'JPY'
  chartId: string
  ...
  stateContract: 'CATEGORICAL_POLARITY_STATE'
  magnitudeState: 'MAGNITUDE_NOT_CONFIGURED'
  guardrails: ChartConditionedPolarityLookup['guardrails']
}

export type SbcAtomicVisibleRangeInterval = {
  interval_id: string
  ...
  guidance_availability: 'AVAILABLE' | 'PARTIAL' | 'UNKNOWN'
  source_cluster_ids: string[]
  missing_evidence_ids: string[]
}

export type SbcAtomicVisibleRange = {
  contract: 'SBC_ATOMIC_VISIBLE_RANGE_V1'
  schema_version: 1
  ...
  intervals: SbcAtomicVisibleRangeInterval[]
  guardrails: {
    read_only: true
...
export type FxSidePilotSideStatus = {
  ...
  reviewedPacketCount: number
  catalogueEntryCount: number
  observedStates: ChartConditionedPolarityState[]
  missingRequiredStates: ChartConditionedPolarityState[]
  unknownGapsRetained: true
  pilotEvidenceComplete: boolean
  blockers: string[]
}

export type FxSidePilotStatus = {
  contract: 'FX_SIDE_POLARITY_PILOT_STATUS_V1'
  schemaVersion: 1
  status: 'PILOT_EVIDENCE_PENDING' | 'PILOT_EVIDENCE_PRESENT_RESEARCH_ONLY'
  requiredStates: Array<'SUPPORTIVE' | 'ADVERSE'>
  eligibleSides: Array<'USD' | 'JPY'>
  sides: Record<'USD' | 'JPY', FxSidePilotSideStatus>
...
export type ChakraTimingSourceVerificationGate = {
  gate_id: string
  state: 'PASS' | 'FAIL' | 'UNKNOWN'
  mandatory: boolean
  label: string
...
export type ChakraTimingSourceArtifactByteCheck = {
  source_id: string
  source_role: string
  lineage_id: string
  ...
  verification_state: 'PASS' | 'FAIL' | 'UNKNOWN'
  detail: string
  hash_method: 'SHA256_EXACT_SUPPLIED_BYTES'
}

export type ChakraTimingExcerptPayloadCheck = {
  claim_id: string
  profile_path: string
  source_id: string
  page_start: number
  ...
  expected_sha256: string
  observed_sha256: string | null
  observed_utf8_byte_length: number | null
  verification_state: 'PASS' | 'FAIL' | 'UNKNOWN'
  detail: string
  hash_method: 'SHA256_EXACT_UTF8_NO_NORMALIZATION'
}

export type ChakraTimingProfileSourceVerificationReport = {
  ...
  packet_id: string | null
  packet_hash: string | null
  s1_ready_for_external_review: boolean
  all_source_bytes_verified: boolean
  all_excerpt_payloads_verified: boolean
  ready_for_independent_review: boolean
  source_artifact_checks: ChakraTimingSourceArtifactByteCheck[]
  excerpt_payload_checks: ChakraTimingExcerptPayloadCheck[]
  validation_gates: ChakraTimingSourceVerificationGate[]
  missing_requirements: string[]
  review_bundle: Record<string, unknown> | null
...
export type ChakraTimingExternalReviewGate = {
  gate_id: string
  state: 'PASS' | 'FAIL' | 'UNKNOWN'
  mandatory: boolean
  label: string
...
export type ChakraTimingProfileExternalReviewReport = {
  ...
  bundle_integrity_verified: boolean
  embedded_s1_ready: boolean
  s2_rows_verified: boolean
  attestation_complete: boolean
  review_approved: boolean
  ready_for_human_certification_decision: boolean
  validation_gates: ChakraTimingExternalReviewGate[]
  missing_requirements: string[]
  certification_proposal: Record<string, unknown> | null
...
export type ChakraTimingSignedReviewGate = {
  gate_id: string
  state: 'PASS' | 'FAIL' | 'UNKNOWN'
  mandatory: boolean
  label: string
...
export type ChakraTimingProfileSignedReviewReport = {
  ...
  s3_ready: boolean
  reviewer_registry_valid: boolean
  reviewer_key_trusted: boolean
  review_signature_valid: boolean
  reviewer_identity_authenticated_to_registry: boolean
  reviewer_independence_administratively_vetted: boolean
  ready_for_manual_source_certification: boolean
  validation_gates: ChakraTimingSignedReviewGate[]
  missing_requirements: string[]
  signed_review_template: Record<string, unknown> | null
  external_review_independently_proven: false
  source_certified: false
  profile_registered: false
  registry_write_allowed: false
