...
its range, orb, prior count, Details action, and Review action. Details and
Analyze Aspect now show timestamp-safe prior-family bullish/bearish rates,
median 72-hour return, neutral upside/downside excursion, bias-conditioned
MFE/MAE when sufficiently sampled, and a compact USD-versus-JPY divergence
view. Evidence badges distinguish versioned geometry, failed or pending
external Shadbala/Drik certification, provisional FX scoring, and retrospective
family labels. Historical outcomes remain excluded from live inference.

Release `0.10.12` renders every visible aspect as a translucent full-height
window, keeps overlapping ribbons independently selectable, and cycles
simultaneous aspects on repeated chart clicks. Its compact hover card shows the
exact range, duration, orb, overlap count, and known historical recurrences.
`Details` expands deterministic Shadbala/Drik evidence plus separate USD and
JPY reference-chart scores; `Review Aspect` opens the existing recurrence,
annotation, local Jyotish, verifier, and Codex workflow. LLM prose remains
draft evidence and is never substituted for the deterministic chart facts.

Release `0.10.11` restores every enabled aspect ribbon in the native chart.
WebView2 could previously paint the Lightweight Charts canvas over the
auto-stacked ribbon layer even though all events were present and clickable.
The isolated chart stack now fixes the canvas, selected window, aspect ribbons,
drawings, and annotations at explicit levels. A source-contract test locks this
ordering, and native QA confirmed all 735 D1-filtered aspect windows both in the
accessibility tree and visibly across the chart.
