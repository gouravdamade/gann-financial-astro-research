...
| KRUSHNA_EVENT_WORKSHEET | Quarantined KAS-specific hypothesis | Event-dependent | Rank event significators and propose Dasha timing | Corrected A/B/C/D/E houses, inverse aspects, Samdharmi transfers/restrictions, D/E bonuses, Vimshottari periods/sectors, nodal proxies, Sun timing and Lesson 26 result multipliers | KRUSHNA_KAS_TIMING lessons 5-11 and 25-27, later corrected Lesson 7 | Implemented only in isolated lab with a passing corrected Lesson 7 fixture and all-12-house USDJPY ablation. No result survived full multiple-testing/placebo/cost checks; exact Krushna ayanamsa and two-calculator certification remain pending. No main-pipeline integration. |
| KRUSHNA_KAS_REVIEW_ADVISORY | Experimental display-only context | Event time | Show the vote across all twelve House B mappings without selecting a trading house | Corrected KAS profile contexts, event time, all-house pair scores and Sun-timed subset | KRUSHNA_KAS_TIMING lessons 5-11 and 25-27; local full-method validation | Implemented only in the repeatation-review drawer through `krushna_kas_advisory.py`. Explicit locks disable trade signals, Auto Suggest input, ML training, rule overrides and MT5. The first USDJPY run found no robust edge. |
