...
class StatusValidationTests(unittest.TestCase):
    ...
    def test_sbc_timing_source_s1_detects_module_hash_drift(self) -> None:
        document = copy.deepcopy(
            _load(
                STATUS_ROOT
    ...
    def test_sbc_timing_source_s1_cannot_claim_source_byte_verification(
        self,
    ) -> None:
        document = copy.deepcopy(
            _load(
        ...
        document["guardrails"]["sourceBytesVerifiedByApplication"] = True
    ...
    def test_sbc_timing_source_s1_cannot_upgrade_readiness_meaning(self) -> None:
        document = copy.deepcopy(
            _load(
                STATUS_ROOT
                / "audits/sbc_timing_profile_source_packet_s1_20260729.json"
        ...
        document["packetReadiness"]["passingStateMeans"] = "SOURCE_CERTIFIED"
        with self.assertRaisesRegex(ValueError, "readiness contract drifted"):
            validate_sbc_timing_profile_source_packet_s1_audit(
                document,
                STATUS_ROOT.parent,
            )

    def test_sbc_timing_source_s1_cannot_claim_certification(self) -> None:
        document = copy.deepcopy(
            _load(
                STATUS_ROOT
                / "audits/sbc_timing_profile_source_packet_s1_20260729.json"
        ...
        document["sourceCertified"] = True
        with self.assertRaisesRegex(ValueError, "sourceCertified must remain false"):
            validate_sbc_timing_profile_source_packet_s1_audit(
                document,
                STATUS_ROOT.parent,
            )

    def test_sbc_timing_source_s2_detects_module_hash_drift(self) -> None:
        document = copy.deepcopy(
            _load(
                STATUS_ROOT
    ...
    def test_sbc_timing_source_s2_bundle_cannot_include_source_bytes(
        self,
    ) -> None:
        document = copy.deepcopy(
            _load(
        ...
        document["reviewBundle"]["includesSourceBytes"] = True
        with self.assertRaisesRegex(ValueError, "review bundle drifted"):
            validate_sbc_timing_profile_source_verification_s2_audit(
                document,
                STATUS_ROOT.parent,
            )

    def test_sbc_timing_source_s2_cannot_claim_page_or_doctrine_review(
        self,
    ) -> None:
        document = copy.deepcopy(
            _load(
        ...
        document["separation"]["doctrineCorrectnessChecked"] = True
        with self.assertRaisesRegex(ValueError, "separation contract drifted"):
            validate_sbc_timing_profile_source_verification_s2_audit(
                document,
                STATUS_ROOT.parent,
    ...
    def test_sbc_timing_source_s2_cannot_unlock_registry_or_execution(
        self,
    ) -> None:
        document = copy.deepcopy(
            _load(
    ...
    def test_sbc_timing_external_review_s3_detects_module_hash_drift(
        self,
    ) -> None:
        document = copy.deepcopy(
            _load(
    ...
    def test_sbc_timing_external_review_s3_proposal_cannot_certify(
        self,
    ) -> None:
        document = copy.deepcopy(
            _load(
        ...
        document["certificationProposal"]["sourceCertified"] = True
        with self.assertRaisesRegex(ValueError, "certification proposal drifted"):
            validate_sbc_timing_profile_external_review_s3_audit(
                document,
                STATUS_ROOT.parent,
            )

    def test_sbc_timing_external_review_s3_cannot_authenticate_reviewer(
        self,
    ) -> None:
        document = copy.deepcopy(
            _load(
        ...
        document["humanControl"]["reviewerIdentityAuthenticated"] = True
    ...
    def test_sbc_timing_external_review_s3_cannot_unlock_registry_or_execution(
        self,
    ) -> None:
        document = copy.deepcopy(
            _load(
    ...
    def test_timing_profile_reviewer_registry_cannot_enable_writes(self) -> None:
        document = copy.deepcopy(
            _load(STATUS_ROOT / "timing_profile_reviewer_trust_registry.json")
        )
        document["registryWriteAllowed"] = True
        with self.assertRaisesRegex(ValueError, "registryWriteAllowed=false"):
            validate_timing_profile_reviewer_trust_registry(document)

    def test_sbc_timing_signed_review_s4_detects_module_hash_drift(
        self,
    ) -> None:
        document = copy.deepcopy(
            _load(
    ...
    def test_sbc_timing_signed_review_s4_rejects_client_public_key(
        self,
    ) -> None:
        document = copy.deepcopy(
            _load(
        ...
        document["trustedReview"]["clientPublicKeyAccepted"] = True
        with self.assertRaisesRegex(ValueError, "trusted-review contract drifted"):
            validate_sbc_timing_profile_signed_review_s4_audit(
                document,
                STATUS_ROOT.parent,
            )

    def test_sbc_timing_signed_review_s4_signature_cannot_certify_source(
        self,
    ) -> None:
        document = copy.deepcopy(
            _load(
        ...
        document["humanControl"]["applicationMayCertifySource"] = True
        with self.assertRaisesRegex(ValueError, "human-control contract drifted"):
            validate_sbc_timing_profile_signed_review_s4_audit(
                document,
                STATUS_ROOT.parent,
    ...
    def test_sbc_timing_signed_review_s4_cannot_unlock_registry_or_execution(
        self,
    ) -> None:
        document = copy.deepcopy(
            _load(
