...
def validate_timing_profile_reviewer_trust_registry(
    document: dict[str, Any],
) -> None:
    label = "timing profile reviewer trust registry"
    if document.get("schemaVersion") != 1:
    ...
    if not isinstance(reviewers, list):
        raise ValueError(f"{label} reviewers must be an array")
    allowed = {
        "reviewerKeyId",
        "publicKeyBase64",
        "reviewerIdentity",
    ...
    for index, item in enumerate(reviewers):
        ...
        key_id = _sha256(
            item.get("reviewerKeyId"),
            f"{entry_label} reviewerKeyId",
        )
        ...
        _required_text(
            item.get("reviewerIdentity"),
            f"{entry_label} reviewerIdentity",
        )
        _required_text(
            item.get("reviewerOrganization"),
            f"{entry_label} reviewerOrganization",
        )
...
def validate_sbc_timing_profile_signed_review_s4_audit(
    document: dict[str, Any], project_root: Path
) -> None:
    label = "SBC timing profile signed review S4"
    _utc_timestamp(document.get("auditedAtUtc"), f"{label} auditedAtUtc")
    _execution_locked(document, label)
    if (
        document.get("capabilityId")
        ...
        raise ValueError(f"{label} has an unexpected capability")
    if document.get("status") != "implemented_in_source_empty_reviewer_registry":
        raise ValueError(f"{label} has an unexpected status")
    for field in (
        "packagedReviewBundle",
        "packagedCompletedAttestation",
        "packagedSignedReview",
        "trustedReviewerRegistryPopulated",
    ...
    implementation = document.get("implementation") or {}
    expected_contracts = {
        "reportContract": "SBC_TIMING_PROFILE_SIGNED_REVIEW_REPORT_V1",
        "signedReviewContract": "SBC_TIMING_PROFILE_SIGNED_REVIEW_V1",
        "reviewerRegistryContract": (
            "SBC_TIMING_PROFILE_REVIEWER_TRUST_REGISTRY_V1"
        ),
        "schemaVersion": 1,
        "signaturePolicy": "ED25519_SERVER_TRUST_REGISTRY_EXACT_S3_BINDING_V1",
        "signatureHashMethod": (
    ...
    expected_trusted_review = {
        ...
        "exactS3EvidenceBindingRequired": True,
        "noSignedReviewState": "NO_SIGNED_REVIEW",
        "s3NotReadyState": "S3_NOT_READY",
        "invalidSignatureState": "SIGNATURE_INVALID",
        "untrustedKeyState": "REVIEWER_KEY_UNTRUSTED",
        "passingState": "READY_FOR_MANUAL_SOURCE_CERTIFICATION",
        "passingStateMeans": "REGISTERED_KEY_SIGNED_EXACT_S3_EVIDENCE_ONLY",
    }
    if (document.get("trustedReview") or {}) != expected_trusted_review:
        raise ValueError(f"{label} trusted-review contract drifted")

    expected_human_control = {
        "registeredKeySignatureAuthenticatesEvidenceBinding": True,
        "reviewerIdentityAuthenticatedWithoutTrustedKey": False,
        "reviewerIndependenceAdministrativelyVettedOnly": True,
        "reviewerIndependenceCryptographicallyProven": False,
        "externalReviewIndependentlyProven": False,
        "manualReviewerKeyEnrollmentRequired": True,
        "manualSourceCertificationDecisionRequired": True,
        "applicationMayEnrollReviewerKey": False,
        "applicationMayCertifySource": False,
        "applicationMayRegisterProfile": False,
        "applicationMayWriteRegistry": False,
    }
    ...
    expected_separation = {
        "s3RecordCoherenceSeparate": True,
        "registeredKeySignatureSeparate": True,
        "reviewerIndependenceProofSeparate": True,
        "sourceCertificationSeparate": True,
        "profileRegistryAdmissionSeparate": True,
        "directionalEngineImplementationSeparate": True,
        "prospectiveFinancialValidationSeparate": True,
        "executionPermissionSeparate": True,
        "repositoryReviewerKeyAvailable": False,
    ...
    expected_transport = {
        "browserDevelopment": "private_http_post",
        "nativeDesktop": "tauri_ipc_private_sidecar",
        "route": "/api/chakra-lab/timing-profile/signed-review/verify",
        "nativeCommand": "chakra_lab_timing_signed_review",
        "readOnlyRuntimeRequired": True,
        "clientSuppliesJsonObjectsOnly": True,
    ...
    for field in (
        "researchOnly",
        "readOnly",
        "serverTrustRegistryRequired",
        "signatureProvesRegisteredKeyBindingOnly",
    ):
        if guardrails.get(field) is not True:
            raise ValueError(f"{label} guardrail {field} must remain true")
    for field in (
        ...
        "reviewerIndependenceCryptographicallyProven",
        "externalReviewIndependentlyProven",
        "sourceCertified",
        "profileRegistered",
        "registryWriteAllowed",
    ...
    expected_verification = {
        "newTimingProfileSignedReviewTests": "11_passed",
        "chakraServiceTests": "22_passed",
        "chakraAuditWorkspaceTests": "9_passed",
        "frontendTests": "101_passed",
        "statusValidation": "45_passed",
        "repositoryPythonTests": "542_passed",
        "pythonRuff": "changed_scope_passed",
        "repositoryWidePythonRuff": "blocked_by_19_out_of_scope_findings",
        "frontendLint": "passed",
        "frontendProductionBuild": "passed",
...
def validate_all(root: Path = STATUS_ROOT) -> dict[str, Any]:
    ...
    if (
        capability_by_id["timing_profile_signed_review_verification_v1"][
            "states"
        ]["implementedInSource"]
        != "yes"
    ):
