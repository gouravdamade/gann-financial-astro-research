...
def validate_cross_document_links(
    ...
    for side, release_key in (
        ...
        if candidate["releaseId"] != selected_release["releaseId"]:
            raise ValueError(f"{side} release ID differs from selected release")
        if candidate["artifactSha256"] != selected_release["artifactSha256"]:
            raise ValueError(f"{side} SHA-256 differs from selected release")
