...
- `release_status.json` separates packaged candidates, physical testing, and
  promotion. The exact desktop `0.10.19` EXE and clean-source Android `0.10.19`
  APK hashes were recomputed successfully.
...
- Android debug `0.10.19`, APK SHA-256
...
built from clean app source at commit `3cabe25`; its debug signer still blocks
production promotion. Execution remains disabled.

The clean candidate build and independent APK inspection are recorded in
...
- clean Android follow-up: frontend 71 passed, backend 117 passed, Rust 15
  passed, strict Clippy passed, Android arm64 assembly passed, APK v2 signature
  and package/native-payload inspection passed.

## Remaining Gates

1. Install the exact clean Android `0.10.19` candidate and collect MOB-01
   through MOB-08 evidence on the physical phone. After behavioral acceptance,
   create a controlled release-signed APK and rerun install/pairing smoke tests
   against the exact promotion artifact.
