...
`tier_b_pyjhora_aligned_pending_independent_jhora_or_worked_example`
...
It contains 35 required rows. The API gate also checks the nested independent
witness and refuses certification when it is absent, incomplete, failed, or
manually inconsistent with the top-level gate.

Accepted next witnesses:

1. a saved Jagannatha Hora export/screenshot with Raman ayanamsa and exact
   fixture settings; or
2. a cited independent worked classical example with reproducible inputs.

Jagannatha Hora 7.6 explicitly offers a choice between Raman and Parasara
special-aspect quantification. Therefore the chosen option must be recorded with
the evidence. No independent values have been invented or copied from PyJHora.
