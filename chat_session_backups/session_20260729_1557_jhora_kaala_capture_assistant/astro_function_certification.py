...
REPORT_VERSION = (
    "astro_certification_4_gate_v12_kaala_capture_assistant_20260729"
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="jhora_hora_boundary_witness_template_20260729.csv",
        help=(
            "Visible case-8 JHora sunrise and Hora-award witness. The pending "
            "template keeps the machine gate blocked."
        ),
    )
    parser.add_argument(
        "--jhora-ayana-intermediate-witness",
        default="jhora_ayana_intermediate_witness_template_20260729.csv",
        help=(
            "Visible historical JHora tropical-longitude or Kranti witness. "
            "The pending template keeps the machine gate blocked."
        ),
    )
    parser.add_argument(
...
def kaala_formula_profile_gate_summary(summary_path: Path) -> dict[str, Any]:
    ...
    expected_contract = "GANN_JHORA_KAALA_FORMULA_PROFILE_RECONCILIATION_V1"
    ...
    base.update(
        {
            ...
            "nextWitnesses": [
                (
                    "Capture JHora sunrise or Hora-award input for case 8; the "
                    "categorical lord changes across a 3.44-minute boundary."
                ),
                (
                    "Capture JHora tropical longitude or intermediate Kranti "
                    "for the five failing 1889 Ayana rows."
                ),
            ],
        }
    )
...
def render_report(
    ...
    lines = [
        ...
        (
            ...
            "of sunrise input. A visible JHora sunrise/intermediate witness is "
            "required before changing Hora."
        ),
        "",
        (
            "The tropical-Kranti Ayana candidate matches all 28 recent rows "
            "and 30/35 overall, but only 2/7 historical rows. It remains a "
            "candidate until JHora tropical longitude or intermediate Kranti "
            "is captured for the five failing 1889 rows."
        ),
        "",
        (
            ...
            "The pending templates require visible, hashed JHora inputs and "
            "keep production changes disabled."
        ),
        "",
        (
            ...
            "infer astrology values, overwrite the pending templates, or "
            "unlock execution."
        ),
        "",
...
def main() -> None:
    ...
    external_gate["requirements"].append(
        ...
        "hashed JHora intermediate-input packet. Pending templates keep "
        "production formula changes disabled."
    )
    external_gate["requirements"].append(
