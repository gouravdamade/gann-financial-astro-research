...
def test_jhora_visible_saptavargaja_respects_moolatrikona_degree_range() -> None:
    longitudes = {
        ...
        "JUPITER": 42.0,
        "VENUS": 355.0,
        "SATURN": 328.73505769172215,
    }
    pyjhora = saptavargaja_bala(
        "SATURN",
        longitudes["SATURN"],
        longitudes,
        profile=SAPTAVARGAJA_PYJHORA_PROFILE,
    )
    visible_jhora = saptavargaja_bala(
        "SATURN",
        longitudes["SATURN"],
        longitudes,
        profile=SAPTAVARGAJA_JHORA_VISIBLE_PROFILE,
    )

    assert visible_jhora["profile"] == SAPTAVARGAJA_JHORA_VISIBLE_PROFILE
    assert (
        visible_jhora["saptavargaja_virupa"]
        == pyjhora["saptavargaja_virupa"] - 15.0
    )
    assert visible_jhora["saptavarga_details"][0]["label"] == "own"
...
def test_luminary_chesta_is_displayed_but_not_double_counted_in_total() -> None:
    ...
    for planet in ("SUN", "MOON"):
        result = shadbala_components_for_planet(
            planet,
            longitudes[planet],
            longitudes,
            0.0,
        ...
        assert float(result["chesta_virupa"]) > 0.0
        assert_close(result["chesta_total_contribution_virupa"], 0.0)
        expected = sum(
            float(result[key])
            for key in (
                "naisargika_virupa",
        ...
        assert_close(result["implemented_total_virupa"], expected)
...
def test_avg_all_component_mean_context() -> None:
    ...
    assert "independent_jhora_doctrine_reconciliation" in ctx[
