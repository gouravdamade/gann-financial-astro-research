...
STRICT_SHADBALA_STATUS = (
    "provisional_independent_jhora_witness_failed_reconciliation_in_progress"
)
...
SAPTAVARGAJA_JHORA_VISIBLE_PROFILE = "jhora_8_visible_compatibility"
...
def saptavargaja_dignity(
    ...
    *,
    ...
    elif profile == SAPTAVARGAJA_JHORA_VISIBLE_PROFILE:
        weights = SAPTAVARGAJA_PYJHORA_VIRUPA_BY_DIGNITY
        is_moolatrikona = (
            varga == "D1" and in_moolatrikona_range(body, d1_longitude)
        )
    else:
...
def chesta_total_contribution_virupa(planet: Any, chesta_virupa: Any) -> float:
    body = normalize_body(planet)
    if body in {"SUN", "MOON"}:
        return 0.0
    try:
        value = float(chesta_virupa)
    except (TypeError, ValueError):
        return np.nan
...
def shadbala_components_for_planet(
    ...
    chesta_total_contribution = chesta_total_contribution_virupa(body, chesta)
    ...
    return {
        ...
        "chesta_total_contribution_virupa": chesta_total_contribution,
...
def aggregate_components(components: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    keys = [
        ...
        "kaala_9_virupa",
        "chesta_virupa",
        "chesta_total_contribution_virupa",
...
def prefix_components(prefix: str, components: dict[str, Any]) -> dict[str, Any]:
    ...
    mapping = {
        ...
        "chesta_total_contribution_virupa": "strict_chesta_total_contribution_virupa",
...
def event_strict_shadbala_context(
    ...
    out["event_strict_shadbala_missing_components"] = (
        "certified_graha_yuddha_disc_diameter_method|independent_jhora_doctrine_reconciliation|"
        "chesta_mean_longitude_model_crosscheck|"
        "ishta_kashta_phala"
    )
    ...
    out["event_strict_shadbala_decision_notes"] = (
        ...
        "Sun/Moon Chesta remains visible but contributes zero to the total because Ayana/Paksha already owns it; "
