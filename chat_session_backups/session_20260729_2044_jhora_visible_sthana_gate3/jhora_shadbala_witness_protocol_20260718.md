...
The original top-level JHora table did not identify which Sthana subcomponent
caused a residual. That evidence gap is now closed by the separate visible
`175/175` row capture under
`jhora_sthana_subcomponent_witness_protocol_20260729.md`. The reconciliation
localizes the production difference to Saptavargaja weights: the named
JHora-visible compatibility profile passes `35/35`, while the separately cited
source profile remains the production research doctrine and passes `1/35`
complete Sthana totals. Compatibility agreement does not authorize a silent
