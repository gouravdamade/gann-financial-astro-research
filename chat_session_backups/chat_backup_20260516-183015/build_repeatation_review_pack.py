...
(function () {{
  ...
  ready(function () {{
    ...
    function setNoteIfEmpty(typeValue, textValue) {{
      var noteTypeNode = panel.querySelector('#repeatation-note-type');
      var noteNode = panel.querySelector('#repeatation-note');
      if (!noteTypeNode.value.trim()) noteTypeNode.value = typeValue;
      if (!noteNode.value.trim()) {{
        noteTypeNode.value = typeValue;
        noteNode.value = textValue;
      }}
    }}
    function noteType() {{
    ...
    function annotationRow(kind) {{
      ...
      return {{
        ...
        kind: kind,
        scope: valueOf('#repeatation-rule-scope', kind === 'rule_note' ? 'case_family' : 'local_window'),
        type: kind === 'ignore_signal'
          ? valueOf('#repeatation-ignore-type', 'nearby_aspect')
          : valueOf('#repeatation-rule-type', 'behavior_rule'),
        note_type: noteType(),
        note: note,
        case_id: meta.caseId,
    ...
    function annotationLedgerHtml() {{
      ...
      return state.annotations.map(function (item, index) {{
        return '<div class="rm-ledger-item">'
          + '<div><b>' + esc(index + 1) + '. ' + esc(item.kind) + '</b> <span>' + esc(item.scope) + ' / ' + esc(item.type) + '</span></div>'
    ...
    function markIgnoreTrade() {{
      ...
      setNoteIfEmpty('ignore_trade_nearby_event', 'ignore trade: nearby/overlapping aspect/event contaminates case behavior');
      drawMarkers();
      render();
      saveDraft();
    }}
    ...
    panel.innerHTML = ''
      ...
      + '<label>Ignore signal type</label><select id="repeatation-ignore-type"><option value="nearby_aspect">nearby_aspect</option><option value="overlapping_aspect">overlapping_aspect</option><option value="crowded_regime">crowded_regime</option><option value="bad_price_data">bad_price_data</option><option value="abnormal_candle">abnormal_candle</option><option value="manual_skip">manual_skip</option></select>'
    ...
    panel.querySelector('#repeatation-ignore-type').addEventListener('change', saveDraft);
