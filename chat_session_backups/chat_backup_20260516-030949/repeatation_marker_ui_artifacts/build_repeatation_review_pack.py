...
(function () {{
  ...
  ready(function () {{
    ...
    function markerShapes() {{
      ...
      function vertical(point, color, dash, name) {{
        if (!point) return;
        shapes.push({{
          type: 'line',
          name: name,
      ...
      vertical(state.tradeStart, '#22c55e', 'solid', 'repeatation-marker-trade-start');
      vertical(state.tradeEnd, '#ef4444', 'solid', 'repeatation-marker-trade-end');
      vertical(state.ignoreStart, '#f97316', 'dash', 'repeatation-marker-ignore-start');
      vertical(state.ignoreEnd, '#f97316', 'dash', 'repeatation-marker-ignore-end');
    ...
    panel.innerHTML = ''
      + '<div class="rm-title">Repeatation Marker UI</div>'
    ...
    style.textContent = ''
      + '#repeatation-marker-panel{{position:fixed;right:14px;top:14px;z-index:9999;width:390px;max-height:92vh;overflow:auto;background:#0f172a;color:#e5e7eb;border:1px solid #475569;border-radius:8px;box-shadow:0 16px 48px rgba(0,0,0,.45);font:12px/1.35 Arial,sans-serif;padding:12px;}}'
      + '#repeatation-marker-panel .rm-title{{font-weight:700;font-size:15px;margin-bottom:4px;}}'
    ...
    gd.addEventListener('click', function (evt) {{
      ...
      setTimeout(function () {{
        if (state.lastPoint && Date.now() - (state.lastPoint.placedAt || 0) < 50) return;
      }}, 0);
...
<body>
  ...
  <main>
    <p class="note">
      Open each chart link and use the fixed <b>Repeatation Marker UI</b> panel. Choose trade start,
      trade end, ignore start, or ignore end, then click the chart to place the marker. The chart overlays
      the selected marker lines/ignore region and generates Python commands for saving trade annotations,
      ignore regions, and rule notes. The annotation command auto-calculates entry, exit, pips, MFE,
      and MAE from the selected price timeframe.
    </p>
    <table>
      <thead>
        <tr>
