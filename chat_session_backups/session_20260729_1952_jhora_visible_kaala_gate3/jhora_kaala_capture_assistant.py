...
from jhora_kaala_intermediate_witness_protocol import (
    ...
    HORA_SAMPLE_ID,
    PLANETS,
    ayana_formula_reconciliation_issues,
    evidence_bundle_issues,
    read_csv,
    validate_ayana_observation_rows,
    validate_hora_rows,
    visible_time_to_decimal_hour,
    witness_gate_summary,
    write_csv,
)
from jhora_witness_protocol import require_pinned_jhora, sha256
from jhora_tropical_position_transcription import ayana_capture_inputs
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--import-ayana-transcription",
        type=Path,
        help=(
            "Validate a seven-planet visible tropical-position transcription "
            "and write the observed Ayana packet without hiding formula "
            "residuals."
        ),
    )
...
def _required_evidence(path: Path) -> Path:
    ...
    issues = evidence_bundle_issues(resolved)
    if issues:
...
def assemble_hora_capture_rows(
    ...
    *,
    ...
    sunrise_display = str(sunrise_lmt_display or "").strip()
    sunrise_text = _number_text(
        visible_time_to_decimal_hour(sunrise_display)
    )
    ...
    for row in rows:
        ...
        row["jhora_sunrise_lmt_display"] = sunrise_display
...
def export_hora_packet(
    *,
    ...
    rows = assemble_hora_capture_rows(
        ...
        evidence_path=evidence_path,
        reviewer=reviewer,
        sunrise_lmt_display=sunrise_lmt_display,
        hora_lord=hora_lord,
        awards=awards,
        captured_at_utc=captured_at_utc,
    )
...
def export_ayana_packet(
    *,
    ...
    issues = validate_ayana_observation_rows(
        rows,
    ...
    if issues:
        raise CaptureValidationError(issues)
    _atomic_write(output_path, rows)
    formula_issues = ayana_formula_reconciliation_issues(rows)
    return {
        "contract": ASSISTANT_CONTRACT,
        "status": (
            "valid_ayana_observation_written_formula_candidate_rejected"
            if formula_issues
            else "valid_ayana_packet_written"
        ),
...
def launch_gui() -> None:
    ...
    ttk.Label(hora_tab, text="Visible sunrise (H:MM:SS)").grid(
    ...
    def save_hora() -> None:
        try:
            result = export_hora_packet(
                ...
                sunrise_lmt_display=sunrise_var.get(),
...
def main() -> int:
    args = parse_args()
    if args.import_ayana_transcription:
        require_pinned_jhora(DEFAULT_JHORA_EXE)
        require_exact_fixture_times()
        capture = ayana_capture_inputs(
            args.import_ayana_transcription,
            args.kaala_witness,
        )
        result = export_ayana_packet(
            evidence_path=cast(Path, capture["evidence_path"]),
            reviewer=cast(str, capture["reviewer"]),
            values=cast(
                Mapping[str, Mapping[str, object]],
                capture["values"],
            ),
            output_path=args.ayana_packet,
            locked_kaala_path=args.kaala_witness,
            captured_at_utc=cast(str, capture["captured_at_utc"]),
        )
