  ...
  ready(function () {{
    ...
    function ignoreCommand() {{
      ...
      return 'python .\\\\aspect_annotation_store.py --mark-ignore-region'
        ...
        + ' --why ' + shellQuote(noteText() || (state.tradeIgnored ? 'ignore trade: nearby/overlapping aspect/event contaminates case behavior' : 'manual repeatation ignore marker'));
    }}
    ...
    function hasDraftableContent() {{
      return !!(state.tradeStart || state.tradeEnd || state.ignoreStart || state.ignoreEnd || state.tradeIgnored || noteText() || state.draftLoaded);
    }}
    function loadDraft() {{
    ...
    panel.innerHTML = ''
      ...
      + '<div class="rm-actions"><button id="repeatation-clear">Clear markers</button><button id="repeatation-clear-draft">Clear saved draft</button><button id="repeatation-download">Download JSON</button></div>'
...
<body>
  ...
  <main>
    ...
    <p class="note">
      ...
      whole case window. The chart overlays crosshair markers/ignore regions and generates Python
