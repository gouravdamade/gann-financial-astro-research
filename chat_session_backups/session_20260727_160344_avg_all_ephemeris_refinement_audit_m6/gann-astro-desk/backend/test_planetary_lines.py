...
def deterministic_longitude(
    ...
    values = [
        30.0 + (timestamp.timestamp() - 1_700_000_000) / 3_600 * 30.0
...
class PlanetaryLineOverlayTests(unittest.TestCase):
    ...
    def test_avg_all_adds_context_only_collective_geometry(self) -> None:
        ...
        self.assertEqual(
            collective["influence"]["contract"],
            "GANN_PLANETARY_COLLECTIVE_INFLUENCE_V1",
        )
        self.assertEqual(len(collective["latest"]["memberAudit"]), 10)
        self.assertFalse(
        ...
        self.assertGreaterEqual(
            ingress["refinedTimeUnix"],
            ingress["sourceBracket"]["startUnix"],
        )
        self.assertLessEqual(
            ingress["refinedTimeUnix"],
            ingress["sourceBracket"]["endUnix"],
        )
        self.assertEqual(
