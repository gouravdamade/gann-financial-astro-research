...
from collective_geometry import (
    AVG_ALL_MEMBERS,
    UNSTABLE_RESULTANT_FLOOR,
    calculate_collective_field,
    legacy_circular_mean,
)
from collective_refinement import refine_collective_events
...
def _collective_longitude_evaluator(
    longitudes: dict[str, np.ndarray],
    timestamps: tuple[int, ...],
) -> Any:
    sampled = {
        float(timestamp): tuple(
            float(longitudes[member][position])
            for member in AVG_ALL_PLANETS
        )
        for position, timestamp in enumerate(timestamps)
    }
    cache: dict[float, tuple[float, ...]] = dict(sampled)

    def evaluate(timestamp: float) -> tuple[float, ...]:
        key = round(float(timestamp), 6)
        cached = cache.get(key)
        if cached is not None:
            return cached
        index = pd.DatetimeIndex(
            [pd.Timestamp(key, unit="s", tz="UTC")]
        )
        values: list[float] = []
        with _EPHEMERIS_LOCK:
            for member in AVG_ALL_PLANETS:
                longitude = fetch_planetary_longitude(
                    member,
                    index,
                    astrology_method="sidereal",
                    coordinate_system="geo",
                ).reindex(index)
                values.append(float(longitude.iloc[0]))
        result = tuple(values)
        cache[key] = result
        return result
...
def build_planetary_line_overlay(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if collective_field is not None:
        refined_events, refined_summary = refine_collective_events(
            collective_field["events"],
            collective_field["eventSummary"],
            longitude_evaluator=_collective_longitude_evaluator(
                longitudes,
                timestamps,
            ),
            unstable_resultant_floor=UNSTABLE_RESULTANT_FLOOR,
        )
        collective_field["events"] = refined_events
        collective_field["eventSummary"] = refined_summary
    for group in groups:
        planet = group["planet"]
        longitude = (
