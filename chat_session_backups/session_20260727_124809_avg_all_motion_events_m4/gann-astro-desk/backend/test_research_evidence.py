...
class ResearchEvidenceContractTests(unittest.TestCase):
    def test_context_packet_cannot_create_a_market_signal(self) -> None:
        ...
        self.assertFalse(packet["guardrails"]["consumedByAutoSuggest"])
        self.assertFalse(packet["guardrails"]["consumedByShadowLedger"])
        self.assertFalse(packet["guardrails"]["consumedByOfficialMlNotes"])
