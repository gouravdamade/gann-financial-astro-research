...
from validation_gates import load_external_gate
...
class AstroRepository:
    def __init__(self, paths: DataPaths | None = None) -> None:
        ...
        self._baseline_reference_touches = self._load_retrospective_touch_frame(
            self.paths.touch_log
        )
        self.external_astrology_gate = load_external_gate(self.paths.project_root)
    ...
    @staticmethod
    def _load_retrospective_touch_frame(path: Path) -> pd.DataFrame:
        header = pd.read_csv(path, nrows=0)
        usecols = [column for column in RETROSPECTIVE_TOUCH_COLUMNS if column in header.columns]
        if "event_id" not in usecols or "touch_time_local" not in usecols:
            return pd.DataFrame(columns=RETROSPECTIVE_TOUCH_COLUMNS)
        frame = pd.read_csv(path, usecols=usecols)
    ...
    def _install_dataset(self, events_path: Path, touch_path: Path, artifact: dict[str, Any]) -> None:
        ...
        retrospective_touches = self._load_retrospective_touch_frame(touch_path)
        ...
        if (
            ...
            known_reference_touches = pd.concat(
                [self._baseline_reference_touches, retrospective_touches],
                ignore_index=True,
            ).drop_duplicates(subset=["event_id"], keep="last")
        else:
            known_events = events.copy()
            known_reference_touches = retrospective_touches.copy()
        ...
        known_price_h1 = price_frames.get("H1")
        if (
            str(artifact.get("symbol") or "").upper() == "USDJPY"
            and str(artifact.get("mode") or "").upper() == "TN"
            and known_price_h1 is not None
        ):
            known_price_h1 = pd.concat(
                [self._baseline_price_by_timeframe["H1"], known_price_h1]
            )
            known_price_h1 = known_price_h1.loc[
                ~known_price_h1.index.duplicated(keep="last")
            ].sort_index()
        known_reference_touch_by_event = {
            str(row["event_id"]): row
            for _, row in known_reference_touches.iterrows()
        }
        with self._data_lock:
            self.events = events
            ...
            self.known_reference_touches = known_reference_touches
            self.known_reference_touch_by_event = known_reference_touch_by_event
            self.known_price_h1 = known_price_h1
    ...
    def _family_summary(occurrences: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        return {
            ...
            "bullishRatePct": round(100.0 * bullish / labeled, 2) if labeled else None,
            "bearishRatePct": round(100.0 * bearish / labeled, 2) if labeled else None,
            "averageReturnPct": round(float(np.mean(returns)), 4) if returns else None,
            "medianReturnPct": round(float(np.median(returns)), 4) if returns else None,
        }

    @staticmethod
    def _localized_timestamp(value: Any, timezone_name: str = IST) -> pd.Timestamp:
        timestamp = pd.Timestamp(value)
        if timestamp.tzinfo is None:
            timestamp = timestamp.tz_localize(timezone_name)
        return timestamp

    def _touch_excursion_72h(self, touch: pd.Series) -> tuple[float, float] | None:
        price = self.known_price_h1
        if price is None or price.empty:
            return None
        anchor = pd.to_numeric(touch.get("close_touch"), errors="coerce")
        if pd.isna(anchor) or float(anchor) <= 0:
            return None
        touch_time = self._localized_timestamp(touch["touch_time_local"]).tz_convert(UTC)
        horizon_end = touch_time + pd.Timedelta(hours=72)
        future = price.loc[(price.index > touch_time) & (price.index <= horizon_end)]
        if future.empty:
            return None
        upside = max(
            0.0,
            (float(pd.to_numeric(future["high"], errors="coerce").max()) / float(anchor) - 1.0)
            * 100.0,
        )
        downside = min(
            0.0,
            (float(pd.to_numeric(future["low"], errors="coerce").min()) / float(anchor) - 1.0)
            * 100.0,
        )
        if not np.isfinite(upside) or not np.isfinite(downside):
            return None
        return upside, downside

    def _historical_family_summary(
        self,
        family_key: str,
        evidence_cutoff: pd.Timestamp,
    ) -> dict[str, Any]:
        cutoff = self._localized_timestamp(evidence_cutoff).tz_convert(UTC)
        family = self.known_events.loc[
            self.known_events["event_family_key"].astype(str) == str(family_key)
        ].sort_values("timestamp")
        prior = family.loc[
            family["timestamp"].map(
                lambda value: self._localized_timestamp(value).tz_convert(UTC)
            )
            < cutoff
        ]
        matured: list[pd.Series] = []
        excluded_immature = 0
        for _, event in prior.iterrows():
            touch = self.known_reference_touch_by_event.get(str(event["event_id"]))
            if touch is None:
                continue
            touch_time = self._localized_timestamp(touch["touch_time_local"]).tz_convert(UTC)
            after72 = touch.get("after72_time_local")
            label_available = (
                self._localized_timestamp(after72).tz_convert(UTC)
                if after72 is not None and not pd.isna(after72)
                else touch_time + pd.Timedelta(hours=72)
            )
            if label_available > cutoff:
                excluded_immature += 1
                continue
            matured.append(touch)

        bullish = sum(
            1 for touch in matured if str(touch.get("ret_after_72h_dir") or "").upper() == "UP"
        )
        bearish = sum(
            1 for touch in matured if str(touch.get("ret_after_72h_dir") or "").upper() == "DOWN"
        )
        labeled = bullish + bearish
        returns = [
            float(value)
            for touch in matured
            if not pd.isna(
                value := pd.to_numeric(touch.get("ret_after_72h_pct"), errors="coerce")
            )
        ]
        excursions = [
            value
            for touch in matured
            if (value := self._touch_excursion_72h(touch)) is not None
        ]
        median_upside = (
            round(float(np.median([value[0] for value in excursions])), 4)
            if excursions
            else None
        )
        median_downside = (
            round(float(np.median([value[1] for value in excursions])), 4)
            if excursions
            else None
        )
        if labeled < 3:
            directional_bias = "insufficient"
        ...
        favorable = None
        adverse = None
        if median_upside is not None and median_downside is not None:
            if directional_bias == "bullish":
                favorable = median_upside
                adverse = abs(median_downside)
            elif directional_bias == "bearish":
                favorable = abs(median_downside)
                adverse = median_upside
        return {
            "contract": "GANN_RETROSPECTIVE_FAMILY_EVIDENCE_V1",
            "asOf": cutoff.isoformat(),
            "scope": "strictly_prior_matured_occurrences",
            "priorOccurrenceCount": int(len(prior)),
            "maturedTouchCount": int(len(matured)),
            "excludedImmatureCount": excluded_immature,
            "labeledCount": labeled,
            "bullish": bullish,
            "bearish": bearish,
            "unknown": int(len(matured) - labeled),
            "bullishRatePct": round(100.0 * bullish / labeled, 2) if labeled else None,
            "bearishRatePct": round(100.0 * bearish / labeled, 2) if labeled else None,
            "averageReturnPct": round(float(np.mean(returns)), 4) if returns else None,
            "medianReturnPct": round(float(np.median(returns)), 4) if returns else None,
            "directionalBias": directional_bias,
            "excursionSampleCount": len(excursions),
            "medianUpsideExcursionPct": median_upside,
            "medianDownsideExcursionPct": median_downside,
    ...
    def _evidence_certifications(
        self,
        pair_evidence: dict[str, Any] | None,
    ) -> list[dict[str, Any]]:
        external = self.external_astrology_gate
        external_status = str(external.get("status") or "")
        if external.get("certified") is True:
            strength_status = "certified"
            strength_label = "Externally certified"
        elif external_status.startswith("failed"):
            strength_status = "failed"
            strength_label = "Failed external gate"
        else:
            strength_status = "blocked"
            strength_label = "Certification blocked"
        strength = dict(external.get("strengthMatrix") or {})
        witness = dict((external.get("independentDrikWitness") or {}).get("rows") or {})
        strength_detail = str(external.get("reason") or "External validation is unavailable.")
        if strength:
            strength_detail += (
                f" Comparator: {int(strength.get('pass') or 0)}/"
                f"{int(strength.get('expectedRows') or 0)} passed."
            )
        if witness:
            strength_detail += (
                f" Independent Drik witness: {int(witness.get('pass') or 0)}/"
                f"{int(witness.get('expected') or 0)} passed, "
                f"{int(witness.get('pending') or 0)} pending."
            )
        pair_available = pair_evidence is not None and pair_evidence.get("status") == "provisional_research_only"
        return [
            {
                "key": "astronomy_geometry",
                "label": "Aspect geometry",
                "status": "versioned",
                "badge": "Versioned calculation",
                "detail": "Swiss Ephemeris Raman sidereal transit-to-natal geometry under the recorded contract.",
                "contract": ASTRO_CONTRACT,
                "sourceSha256": None,
            },
            {
                "key": "shadbala_drik",
                ...
                "badge": strength_label,
                "detail": strength_detail,
                "contract": str(external.get("contract") or "GANN_ASTRO_EXTERNAL_CERTIFICATION_GATE_V1"),
                "sourceSha256": external.get("sourceSha256"),
            },
            {
                "key": "currency_pair",
                "label": "Forex pair scoring",
            ...
            },
            {
                ...
                "badge": "Historical labels",
                "detail": "Only prior 72-hour labels that had matured before the selected event are included.",
                "contract": "GANN_RETROSPECTIVE_FAMILY_EVIDENCE_V1",
                "sourceSha256": None,
            },
        ]
    ...
    def event_detail(self, event_id: str) -> dict[str, Any]:
        ...
        historical_family = self._historical_family_summary(
            str(row["event_family_key"]),
        ...
        return {
            ...
            "historicalFamilySummary": historical_family,
            "currencyPairEvidence": pair_evidence,
            "evidenceCertifications": self._evidence_certifications(pair_evidence),
            "context": {
