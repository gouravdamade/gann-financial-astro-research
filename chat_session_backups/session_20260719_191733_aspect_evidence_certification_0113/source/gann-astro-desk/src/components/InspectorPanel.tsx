...
import { CurrencyDivergence } from './CurrencyDivergence'
import { EvidenceCertificationGrid } from './EvidenceCertificationGrid'
import { HistoricalFamilyEvidence } from './HistoricalFamilyEvidence'
  ...
  return (
    <aside className="inspector-panel">
      ...
      {detailsOpen && (
        <>
          <section className="inspector-section">
            <div className="section-title"><CheckCircle2 size={15} /> Evidence status</div>
            {detail ? (
              <EvidenceCertificationGrid items={detail.evidenceCertifications} />
            ) : <span className="muted">Loading certification status...</span>}
          </section>
          <section className="inspector-section">
            <div className="section-title"><Scale size={15} /> Family recurrence</div>
            {detail ? (
              <>
                <dl className="property-grid loaded-family-summary">
                  <dt>Loaded</dt><dd>{detail.familySummary.total} occurrences</dd>
                  <dt>Reviewed</dt><dd>{detail.familySummary.reviewed} of {detail.familySummary.total}</dd>
                  <dt>All labels</dt><dd>{detail.familySummary.bullish} up / {detail.familySummary.bearish} down / {detail.familySummary.unknown} unknown</dd>
                  <dt>All median</dt><dd>{detail.familySummary.medianReturnPct == null ? 'not available' : `${detail.familySummary.medianReturnPct > 0 ? '+' : ''}${detail.familySummary.medianReturnPct.toFixed(3)}%`}</dd>
                </dl>
