import { Crosshair, Pin, PinOff, ShieldCheck, X } from 'lucide-react'
...
export function CollectiveFieldInspector({
  ...
  const nearbyEvents = useMemo(
    () => [...field.events]
      .sort((left, right) => (
        Math.abs(left.estimatedTimeUnix - selected.time)
        - Math.abs(right.estimatedTimeUnix - selected.time)
      ))
      .slice(0, 4),
      ...
      <div className="collective-field-inspector-body">
        <div className="collective-field-plot-pane">
          ...
          >
            ...
            <desc id="collective-field-chart-description">
              Wrapped mean longitude, coherence R1, polarisation R2, variance, velocity, reliability, and approximate sampled event markers.
            </desc>
            ...
            {field.events.map((event) => (
              <g key={event.eventId}>
                <line
                  x1={xFor(event.estimatedTimeUnix)}
                  x2={xFor(event.estimatedTimeUnix)}
                  y1={314}
                  y2={340}
                  className={`collective-field-event is-${event.eventType.toLowerCase()}`}
                />
                <circle
                  cx={xFor(event.estimatedTimeUnix)}
                  cy={326}
                  r={4}
                  className={`collective-field-event-dot is-${event.eventType.toLowerCase()}`}
                >
                  <title>{eventLabel(event)} | approximate sampled time</title>
                </circle>
              </g>
            ))}
          ...
          <div className="collective-field-time-control">
            ...
            <span>{field.events.length} approximate events</span>
          </div>
        </div>
        ...
        <aside className="collective-field-audit-pane" aria-label="Selected collective timestamp audit">
          ...
          <div className="collective-field-event-list">
            <strong>Nearest sampled events</strong>
            {nearbyEvents.length ? nearbyEvents.map((event) => (
              <div key={event.eventId}>
                <span>{eventLabel(event)}</span>
                <time dateTime={new Date(event.estimatedTimeUnix * 1000).toISOString()}>
                  {new Date(event.estimatedTimeUnix * 1000).toLocaleString()}
                </time>
                <small>estimated between exact samples</small>
              </div>
            )) : <span>No sampled event in this range</span>}
          </div>
          <footer>
            <ShieldCheck size={13} />
