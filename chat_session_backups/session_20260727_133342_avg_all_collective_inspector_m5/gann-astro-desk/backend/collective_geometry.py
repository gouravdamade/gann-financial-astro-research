...
def calculate_collective_field(
    ...
    for position, raw_timestamp in enumerate(timestamps):
        ...
        sample["memberAudit"] = build_member_audit(
            column,
            members=AVG_ALL_MEMBERS,
            weights=[1.0 / len(AVG_ALL_MEMBERS)] * len(AVG_ALL_MEMBERS),
            mean_longitude_deg=sample["meanLongitudeDeg"],
            coherence_r1=sample["coherenceR1"],
            longitude_reliable=sample["longitudeReliable"],
    ...
    samples, motion = apply_reliability_safe_motion(samples)
    influence = summarize_member_influence(samples)
    profile = _profile_payload()
    profile["memberSetHash"] = _member_set_hash()
    events, event_summary = detect_sampled_collective_events(
        samples,
        profile_id=LEGACY_AVG_ALL_PROFILE_ID,
        low_coherence_floor=LOW_COHERENCE_FLOOR,
        concentrated_floor=CONCENTRATED_FLOOR,
    )
    latest = samples[-1]
    context_reason = (
    ...
    evidence = build_context_only_evidence_packet(
        ...
        descriptors=[
            ...
            {
                "key": "velocity_deg_per_day",
                "label": "Reliability-safe velocity",
                "value": latest["velocityDegPerDay"],
                "unit": "degree_per_day",
                "status": latest["reliability"],
            },
            {
                "key": "acceleration_deg_per_day2",
                "label": "Reliability-safe acceleration",
                "value": latest["accelerationDegPerDay2"],
                "unit": "degree_per_day2",
                "status": latest["reliability"],
            },
        ],
        provenance={
    ...
    return {
        ...
        "causalClusterPolicy": {
            ...
            "description": (
                "Individual longitudes, aspects, AVG mean, R1, R2, motion, and "
                "member influence/events are linked descriptors of one "
                "planetary-geometry source."
            ),
