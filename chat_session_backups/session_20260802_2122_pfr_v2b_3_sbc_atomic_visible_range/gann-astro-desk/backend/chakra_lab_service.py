...
from sbc.timing_profile_external_review import (  # noqa: E402
...
SBC_ATOMIC_VISIBLE_RANGE_CONTRACT = "SBC_ATOMIC_VISIBLE_RANGE_V1"
SBC_ATOMIC_VISIBLE_RANGE_SCHEMA_VERSION = 1
...
TIMING_EXTERNAL_REVIEW_REQUEST_KEYS = {"reviewBundle", "attestation"}
TIMING_SIGNED_REVIEW_REQUEST_KEYS = {
    "reviewBundle",
    "attestation",
    "signedReview",
}
...
def _guidance_availability(interval: Any) -> str:
    summary = interval.total_summary
    if not summary.scored_contribution_count:
        return "UNKNOWN"
    if summary.unknown_contribution_count or summary.missing_evidence_count:
        return "PARTIAL"
    return "AVAILABLE"


def build_chakra_lab_atomic_range(payload: Any) -> dict[str, Any]:
    """Expose the existing SBC atomic timeline without deriving market polarity."""
    ledger = _build_chakra_lab_ledger(payload)
    return {
        "contract": SBC_ATOMIC_VISIBLE_RANGE_CONTRACT,
        "schema_version": SBC_ATOMIC_VISIBLE_RANGE_SCHEMA_VERSION,
        "classification": ledger.classification,
        "instrument_identity": ledger.instrument_identity,
        "range_start_utc": ledger.range_start_utc.isoformat(),
        "range_end_utc": ledger.range_end_utc.isoformat(),
        "source_ledger_id": ledger.ledger_id,
        "source_atomic_series_id": ledger.source_atomic_series_id,
        "profile_identity": to_primitive(ledger.profile_identity),
        "source_ids": list(ledger.source_ids),
        ...
        "intervals": [
            {
                ...
                "evidence_cutoff_utc": interval.evidence_cutoff_utc.isoformat(),
                "classification": interval.classification,
                "guidance_availability": _guidance_availability(interval),
                "ledger_summary": to_primitive(interval.total_summary),
                "source_cluster_ids": [
                    cluster.cluster_id for cluster in interval.causal_clusters
                ],
                "missing_evidence_ids": [
                    cluster.missing_evidence_id
                    for cluster in interval.causal_clusters
                    if cluster.missing_evidence_id
                ],
            }
            for interval in ledger.interval_ledgers
        ],
        "guardrails": {
            **to_primitive(ledger.guardrails),
            "read_only": True,
...
def build_chakra_lab_timing_external_review(
    payload: Any,
) -> dict[str, Any]:
    if not isinstance(payload, dict):
        raise ValueError("timing external review request must be an object")
    _reject_unknown(
        payload,
        TIMING_EXTERNAL_REVIEW_REQUEST_KEYS,
        "timing external review request",
    )
    return SbcTimingProfileExternalReviewVerifier().compile(
        payload.get("reviewBundle"),
        payload.get("attestation"),
    ).to_dict()


def build_chakra_lab_timing_signed_review(
    payload: Any,
) -> dict[str, Any]:
    if not isinstance(payload, dict):
        raise ValueError("timing signed review request must be an object")
    _reject_unknown(
        payload,
        TIMING_SIGNED_REVIEW_REQUEST_KEYS,
        "timing signed review request",
    )
    registry_path = (
        PROJECT_ROOT
        / "status"
    ...
    registry = json.loads(registry_path.read_text(encoding="utf-8"))
    return SbcTimingProfileSignedReviewVerifier().compile(
        payload.get("reviewBundle"),
        payload.get("attestation"),
