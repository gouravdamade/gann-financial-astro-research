...
STRICT_SHADBALA_RULE_ID = "STRICT_SHADBALA_V6_BPHS_COMPONENT_RECONCILIATION_PROVISIONAL"
STRICT_DRIK_RULE_ID = DRIK_ENGINE_RULE_ID
STRICT_SHADBALA_STATUS = (
    "provisional_bphs_source_profile_with_astronomical_kaala_and_mean_longitude_chesta_pending_jhora_witness"
)
...
def shadbala_components_for_planet(
    ...
    saptavargaja_comparator = saptavargaja_bala(
        body,
        value,
        longitudes,
    ...
    sthana_partial = (
        float(sum(float(v) for v in sthana_partial_values))
        if all(np.isfinite(float(v)) for v in sthana_partial_values)
        else np.nan
    )
    sthana_comparator_values = [
        uchcha,
        saptavargaja_comparator.get("saptavargaja_virupa", np.nan),
        ojayugma,
        kendradi,
        drekkana,
    ]
    sthana_comparator = (
        float(sum(float(v) for v in sthana_comparator_values))
        if all(np.isfinite(float(v)) for v in sthana_comparator_values)
        else np.nan
    )
    implemented_values = [
